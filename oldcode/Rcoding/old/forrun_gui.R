...
rawTEJ <- read_excel(path = "DB.xlsx", sheet = "DB_recol", col_names = TRUE, col_types = as.character(colattr$type))

# rename columns
setnames(rawTEJ, old=as.character(colattr$old), new=as.character(colattr$new))
...
# GDP : ln(realGDP)
rGDP <- read_excel("DB.xlsx", sheet="GDP", skip = 1)
