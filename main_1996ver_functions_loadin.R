...
DBfilter <- function(x = TEJ,filt='filtered'){
  ...
  DB3 <- DB2[!(DB2$FAMILY %in% NA) & # most family with NA got lots of NAs in other columns
               ...
               !(DB2$NetSales %in% NA) & # remove netsales = 0 ... Denominator of (RD,EMP,MARKET),HHI's main var,
               !(DB2$employee %in% NA)]
  DB4 <- rbind(DB0,DB3)
...
fn_relation <- function(x=TEJ3){
  y <- transform(x,RELAT = ifelse(!is.finite(RELATIN/RELATOUT),0,RELATIN/RELATOUT))
  }
