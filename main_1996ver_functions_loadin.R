...
DBfilter <- function(x = TEJ,out=c("csv","txt")){
  x$year <- year(x$date)
  ...
  y.na0.2001 <- x.filt.ind.2001 %>% filter(FAMILY %in% NA | PB %in% NA | TA %in% NA | NetSales %in% c(NA,0) | employee %in% c(NA,0) | is.na(QUICK) | is.na(OUTINSTI))
  y.full.2001 <- x.filt.ind.2001 %>% filter(!is.na(FAMILY) & !is.na(PB) & !is.na(TA) & !NetSales %in% c(NA,0) & !employee %in% c(NA,0) & !is.na(QUICK) & !is.na(OUTINSTI))
  y.full.big5.2001 <- y.full.2001 %>% group_by(TSE_code,year) %>% filter(NROW(TSE_code) >= 5)
  y.full.small5.2001 <- y.full.2001 %>% group_by(TSE_code,year) %>% filter(NROW(TSE_code) < 5)
...
fnMNC <- function(x=){}
  ...
  outli <- function(x){
    ...
    mad1 <- mad(x,na.rm = TRUE)
    mad2 <- median(abs(x - med1))
    return(abs(x - med1)/(mad1/0.6745))}
winsamp <- function(x = 'TEJ82', col=c('ETR','CETR','ROA','SIZE','LEV','INTANG','QUICK','EQINC','OUTINSTI','RELAT')
                     , prob=0.01){
