...
# TEJ8 = original data
# TEJ8.1 = winsorized and make >1 one, <0 zero.
# TEJ8.1.1 = select only year 2006:2015

yr <- seq(2006,2015)
TEJ8.1.1 <- TEJ8.1 %>% filter(year %in% yr)
...
write(stargazer(
  TEJ8.1.1 %>% select(ETR,CETR,STR,HHI_Dum,STR_HHI,ROA,SIZE,LEV,
                    INTANG,QUICK,EQINC,OUTINSTI,RELAT,FAM_Dum,GDP,
                    RD,EMP,MB,MARKET,PPE,
                    STR_RD,STR_EMP,STR_MB,STR_MARKET,STR_PPE
  )
...
# ----
yr2 <- seq(2001,2005)
TEJ8.1.2 <- TEJ8.1 %>% filter(year %in% yr2)

write(stargazer(
  TEJ8.1.2 %>% select(ETR,CETR,STR,HHI_Dum,STR_HHI,ROA,SIZE,LEV,
                    INTANG,QUICK,EQINC,OUTINSTI,RELAT,FAM_Dum,GDP,
                    RD,EMP,MB,MARKET,PPE,
                    STR_RD,STR_EMP,STR_MB,STR_MARKET,STR_PPE
  )
...
ETR.cortab <- corstars(TEJ8.1.1 %>% select(ETR,STR,HHI_Dum,ROA,SIZE,LEV,
                                         INTANG,QUICK,EQINC,OUTINSTI,RELAT,FAM_Dum,GDP)
                       ,method = "pearson",removeTriangle = "lower",star = 3,result = "none")
CETR.cortab <- corstars(TEJ8.1.1 %>% select(CETR,STR,HHI_Dum,ROA,SIZE,LEV,
                                          INTANG,QUICK,EQINC,OUTINSTI,RELAT,FAM_Dum,GDP)
                        ,method = "pearson",removeTriangle = "lower",star = 3,result = "none")
...
lm.CETR <- lm(CETR ~ STR+HHI_Dum+ROA+SIZE+LEV+INTANG+QUICK+EQINC+OUTINSTI+RELAT+FAM_Dum+GDP
              ,TEJ8.1.1)
lm.ETR.SH <- lm(ETR ~ STR+HHI_Dum+STR_HHI+ROA+SIZE+LEV+INTANG+QUICK+EQINC+OUTINSTI+RELAT+FAM_Dum+GDP,TEJ8.1.1)
lm.CETR.SH <- lm(CETR ~ STR+HHI_Dum+STR_HHI+ROA+SIZE+LEV+INTANG+QUICK+EQINC+OUTINSTI+RELAT+FAM_Dum+GDP,TEJ8.1.1)
#############multiple linear model########################
...
# ----
TEJ8.1.1$STR_HHIsum <- TEJ8.1.1$STR * TEJ8.1.1$HHIsum
...
lm.CETR.n1 <- lm(CETR ~ STR+HHIsum+ROA+SIZE+LEV+INTANG+QUICK+EQINC+OUTINSTI+RELAT+FAM_Dum+GDP
                 ,TEJ8.1.1)
lm.ETR.SH.n1 <- lm(ETR ~ STR+HHIsum+STR_HHIsum+ROA+SIZE+LEV+INTANG+QUICK+EQINC+OUTINSTI+RELAT+FAM_Dum+GDP,TEJ8.1.1)
lm.CETR.SH.n1 <- lm(CETR ~ STR+HHIsum+STR_HHIsum+ROA+SIZE+LEV+INTANG+QUICK+EQINC+OUTINSTI+RELAT+FAM_Dum+GDP,TEJ8.1.1)
...
ETR.origHHIsum <- corstars(TEJ8.1.1 %>% select(ETR,STR,HHIsum,ROA,SIZE,LEV,
                                             INTANG,QUICK,EQINC,OUTINSTI,RELAT,FAM_Dum,GDP)
                           ,method = "pearson",removeTriangle = "lower",star = 3,result = "none")
CETR.origHHIsum <- corstars(TEJ8.1.1 %>% select(CETR,STR,HHIsum,ROA,SIZE,LEV,
                                              INTANG,QUICK,EQINC,OUTINSTI,RELAT,FAM_Dum,GDP)
                            ,method = "pearson",removeTriangle = "lower",star = 3,result = "none")
...
# ----
TEJ8.2 <- TEJ8.1.1 %>% mutate(STR.PR = STR_RD.perank+STR_MB.perank+STR_EMP.perank+STR_PPE.perank+STR_MARKET.perank) %>% mutate(STR.PR_HHI = STR.PR*HHI_Dum)
...
TEJ8.3 <- merge(
  merge(
    x=TEJ8.1.1 %>% group_by(TSE_code,year) %>% summarise(NetSales.Deno = sum(as.numeric(NetSales),na.rm=TRUE)),
    y=TEJ8.1.1 %>% group_by(TSE_code,year) %>% top_n(n=4,wt = NetSales) %>% summarise(NetSales.nume = sum(as.numeric(NetSales),na.rm=TRUE)),
    by=c("TSE_code","year")) %>%
